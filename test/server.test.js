...
function post(path, body, token) {
  ...
  return request({ hostname: '127.0.0.1', port: 3000, path, method: 'POST', headers }, payload);
}

function get(path, token) {
  const headers = {};
  if (token) headers['Authorization'] = 'Bearer ' + token;
  return request({ hostname: '127.0.0.1', port: 3000, path, method: 'GET', headers });
}
...
function put(path, body, token) {
  ...
  const headers = { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) };
  if (token) headers['Authorization'] = 'Bearer ' + token;
  return request({ hostname: '127.0.0.1', port: 3000, path, method: 'PUT', headers }, payload);
}
